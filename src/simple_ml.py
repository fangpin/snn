...
def softmax_regression_epoch(X, y, theta, lr=0.1, batch=100):
    ...
    for i in range(X.shape[0] // batch):
        offset = batch * i
        x_batch = X[offset : offset + batch, :]

        y_batch = y[offset : offset + batch]

        z = np.exp(np.matmul(x_batch, theta))

        normlized_z = z / np.sum(z, axis=1)[:, np.newaxis]

        I_y = np.zeros((batch, theta.shape[1]), dtype=np.float32)
        I_y[np.arange(batch), y_batch] = 1.0

        loss = (x_batch.T @ (normlized_z - I_y)) / batch
...
def Iy(y, k, t=np.float32):
    ret = np.zeros((y.shape[0], k), t)
    ret[np.arange(y.shape[0]), y] = 1.0
    return ret
...
def nn_epoch(X, y, W1, W2, lr=0.1, batch=100):
    ...
    for i in range(X.shape[0] // batch):
        ...
        y_batch = y[offset : offset + batch]
        z1 = relu(x_batch @ W1)
        g2 = normlize(np.exp(z1 @ W2)) - Iy(y_batch, W2.shape[1])
...
if __name__ == "__main__":
    ...
    print("\nTraining softmax regression with self implemented cpp")
